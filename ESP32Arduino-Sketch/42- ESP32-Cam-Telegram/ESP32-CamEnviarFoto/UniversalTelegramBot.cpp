...
String UniversalTelegramBot::sendGetToTelegram(const String& command) {
  ...
  if (!client->connected()) {
        ...
        Serial.println(F("[BOT]Conectando al servidor"));
    #endif
    if (!client->connect(TELEGRAM_HOST, TELEGRAM_SSL_PORT)) {
      #ifdef TELEGRAM_DEBUG  
        Serial.println(F("[BOT]Error en la conexión"));
      #endif
    }
  }
  if (client->connected()) {
...
String UniversalTelegramBot::sendPostToTelegram(const String& command, JsonObject payload) {
  ...
  if (!client->connected()) {
    #ifdef TELEGRAM_DEBUG  
        Serial.println(F("[BOT Client]Conectando al servidor"));
    #endif
    if (!client->connect(TELEGRAM_HOST, TELEGRAM_SSL_PORT)) {
      #ifdef TELEGRAM_DEBUG  
        Serial.println(F("[BOT Client]Error de conexión"));
      #endif
    }
  }
  if (client->connected()) {
